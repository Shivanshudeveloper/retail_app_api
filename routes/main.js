...
const MobileProduct_Model = require('../models/MobileProduct');
...
router.get('/delproject/:id', (req, res) => {
    const { id } = req.params;
    Project_Model.findOneAndDelete({ '_id': id })
        .then(data => {
            res.status(200).json('Removed')
        })
        .catch(err => res.status(400).json(`Error: ${err}`))
...
router.post('/addmobileproduct', (req, res) => {
    const body = req.body
    ...
    const mobProduct = new MobileProduct_Model(body)
...
router.get('/getmobileproductbyid/:id', (req, res) => {
    let { id } = req.params;
    MobileProduct_Model.find({ 'pdId': id })
        .then(data => {
            res.status(200).json(data);
        })
        .catch(err => res.status(400).json(`Error: ${err}`))
...
router.get('/delmobileproductbyid/:id', (req, res) => {
    let { id } = req.params;
    MobileProduct_Model.deleteMany({ 'pdId': id })
        .then(data => {
            res.status(200).json(data);
        })
        .catch(err => res.status(400).json(`Error: ${err}`))
...
router.get('/getmobileproductbyidreal/:id', (req, res) => {
    let { id } = req.params;
    MobileProduct_Model.find({ '_id': id })
        .then(data => {
            res.status(200).json(data);
        })
        .catch(err => res.status(400).json(`Error: ${err}`))
...
router.get('/getallprojectsmobile', (req, res) => {
    MobileProduct_Model.find({},(err, data_) => {
        if (err) {
            return res.status(400).json({ success: false, error: err })
        }
        if (!data_) {
